...
DSPAASR adds native anti-aliasing controls, NVIDIA DLAA/DLSS, AMD FSR 3.1.5 Native AA/Super Resolution, and DLSS/FSR frame generation to the game's graphics settings. Backend availability is checked on the game's rendering device; compatibility with every GPU, driver, game build and mod combination is not guaranteed. Moving objects, particles and animated materials can show artifacts.
...
Both FG backends use a same-adapter D3D12 presentation bridge while Unity remains on D3D11. Frame generation consumes GPU time, and the integration also adds input preparation, transport and presentation costs. Enabling it can materially reduce the application's base rendering frame rate; assess the net smoothness and input latency on the actual hardware and scene rather than treating the requested multiplier as a guaranteed performance gain.
