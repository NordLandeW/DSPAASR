...
**Frame generation** offers Off, analytical **FSR 2×** and **DLSS**, independently of the AA/SR choice. DLSS additionally exposes a frame multiplier and NVIDIA Reflex. FSR generates at most one additional frame. DLSS multiplier/Dynamic choices follow the running SDK and device capabilities. Multi Frame Generation and Dynamic require compatible RTX 50-series hardware and SDK support; in-game compatibility for these modes remains unverified. DLSS FG requires Reflex to be enabled. These are requested settings, not a guarantee that the SDK generates or displays a frame for every application frame.

The presentation bridge adds copies and synchronization even while generation itself is stopped. To avoid that cost for SR-only users, the preloader leaves the game's native presentation path unchanged when the saved FG backend is Off. In that session, selecting FG and applying saves a request for the next start: restart to create the bridge and check actual backend support. A selectable next-start request does not mean hardware support has already been established; additional DLSS multipliers and Dynamic stay gated until the SDK reports them.
