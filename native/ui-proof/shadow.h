...
struct ShadowStats {
    ...
    uint64_t iaReadHits = 0, iaReadMisses = 0, iaResidentPages = 0, iaEvictedPages = 0;
    uint64_t iaWcBytes = 0, iaWcCopies = 0;
...
class ConstantShadow final : public capture::WriteObserver {
  public:
    ...
    // Large-IA cache policy, not a supported buffer-size or game-layout limit.
    // Resident data, per-byte validity and nodes all share maximumBytes above.
    static constexpr unsigned iaPageBytes = 4096, maximumIaPages = 8;
