...
struct Image {
    ...
    void transition(ID3D12GraphicsCommandList* list, D3D12_RESOURCE_STATES next) {
        Dx11Dx12::transition(list, resource.Get(), state, next);
        state = next;
    }
};
struct Slot {
...
bool texture(const PresentImage& image) {
    ...
    return description.Dimension == D3D12_RESOURCE_DIMENSION_TEXTURE2D && description.Width &&
           description.Height && description.DepthOrArraySize == 1 && description.MipLevels == 1 &&
...
sl::Boolean boolean(bool value) {
    return value ? sl::eTrue : sl::eFalse;
}
sl::float3 vector(const std::array<float, 3>& value) {
    return {value[0], value[1], value[2]};
}
sl::float4x4 matrix(const std::array<float, 16>& value) {
    sl::float4x4 result;
    for (unsigned row = 0; row < 4; ++row)
        result[row] = {value[row * 4], value[row * 4 + 1], value[row * 4 + 2], value[row * 4 + 3]};
    return result;
}
struct ContentView {
    sl::Extent area;
    float cameraAspect = 0;
};
bool contentArea(const RECT& rectangle, unsigned width, unsigned height, sl::Extent& area) {
    if (!rectangle.left && !rectangle.top && !rectangle.right && !rectangle.bottom) {
        area = {0, 0, width, height};
        return width && height;
    }
    // Validate signed coordinates before subtraction/conversion. Only the all-zero
    // sentinel selects full output; degenerate or inverted rectangles never do.
    if (rectangle.left < 0 || rectangle.top < 0 || rectangle.right <= rectangle.left ||
        rectangle.bottom <= rectangle.top || static_cast<unsigned>(rectangle.right) > width ||
        static_cast<unsigned>(rectangle.bottom) > height)
        return false;
    area = {static_cast<unsigned>(rectangle.top), static_cast<unsigned>(rectangle.left),
            static_cast<unsigned>(rectangle.right - rectangle.left),
            static_cast<unsigned>(rectangle.bottom - rectangle.top)};
    return true;
}
bool projectionAspect(const std::array<float, 16>& projection, float& aspect) {
    // Perspective x/y scales belong to the actual camera, not the physical
    // swapchain or a rounded SR raster. A clip-space Y reflection changes sign,
    // not camera aspect; row-major/transposed storage preserves these diagonals.
    const float x = std::abs(projection[0]), y = std::abs(projection[5]);
    if (!std::isfinite(x) || !std::isfinite(y) || x == 0 || y == 0)
        return false;
    aspect = y / x;
    return std::isfinite(aspect) && aspect > 0;
}
// Format conversion only: HUDless is already postprocessed in Final's encoding.
// Floating-point storage is not a declaration of linear-light values.
...
ComPtr<ID3DBlob> compile(const char* entry, const char* profile) {
    ...
    const auto result = D3DCompile(
        blitShader, sizeof(blitShader) - 1, "DSPAASR SL HUDless conversion", nullptr, nullptr, entry, profile,
...
struct SlPresenter::Impl {
    ...
    void tag(Slot& slot, const PresentationFrame& frame, const ContentView& view);
    ...
    bool usable() const {
        return !stopped && !runtime->quarantined && chain;
    }
};
void SlPresenter::Impl::initialize() {
    ...
    graphicsCheck(runtime->proxyDevice->CreateCommandQueue(&queueDescription, IID_PPV_ARGS(&queue)),
                  "Create Streamline proxy presentation queue");
    ...
    graphicsCheck(runtime->proxyFactory->CreateSwapChainForHwnd(
                      queue.Get(), creation.window, &description,
                      creation.fullscreen ? &*creation.fullscreen : nullptr, nullptr, &created),
                  "Create Streamline proxy swapchain");
    ...
    if (!native || native == chain.Get())
        throw std::runtime_error("Streamline did not proxy the presentation chain");
    ...
    description.Width = actual.Width;
    description.Height = actual.Height;
    description.Format = actual.Format;
    graphicsCheck(runtime->device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&copiedFence)),
                  "Create Streamline copy fence");
    event = CreateEventW(nullptr, FALSE, FALSE, nullptr);
    if (!event)
        graphicsCheck(HRESULT_FROM_WIN32(GetLastError()), "Create Streamline fence event");
    createPipeline();
    query();
...
void SlPresenter::Impl::createPipeline() {
    ...
    graphicsCheck(D3D12SerializeRootSignature(&rootDescription, D3D_ROOT_SIGNATURE_VERSION_1, &blob, &errors),
                  "Serialize SL blit root signature");
    graphicsCheck(runtime->device->CreateRootSignature(0, blob->GetBufferPointer(), blob->GetBufferSize(),
                                                       IID_PPV_ARGS(&root)),
                  "Create SL blit root signature");
    ...
    state.RTVFormats[0] = sampleFormat(description.Format);
    state.SampleDesc.Count = 1;
    graphicsCheck(runtime->device->CreateGraphicsPipelineState(&state, IID_PPV_ARGS(&pipeline)),
                  "Create SL HUDless conversion pipeline");
}
bool SlPresenter::Impl::completed(ID3D12Fence* fence, uint64_t value) {
    if (!value)
        return true;
    if (!fence)
        throw std::runtime_error("A Streamline retirement value has no fence");
    ...
    if (completedValue == UINT64_MAX)
        throw std::runtime_error("Streamline fence reports device removal");
...
void SlPresenter::Impl::wait(ID3D12Fence* fence, uint64_t value) {
    ...
    while (!completed(fence, value)) {
        ...
        if (elapsed >= detail::slTimeoutMilliseconds)
            throw std::runtime_error("Streamline GPU retirement timed out");
        const auto result =
            WaitForSingleObject(event, static_cast<DWORD>(detail::slTimeoutMilliseconds - elapsed));
        if (result != WAIT_OBJECT_0)
            throw std::runtime_error("Streamline GPU retirement wait failed or timed out");
...
void SlPresenter::Impl::options(SlGenerationMode mode, const SlGenerationRequest& request) {
    ...
    value.mode = mode == SlGenerationMode::Fixed     ? sl::DLSSGMode::eOn
                 : mode == SlGenerationMode::Dynamic ? sl::DLSSGMode::eDynamic
                                                     : sl::DLSSGMode::eOff;
    ...
    // HUDless is supplied without an application-produced UI alpha/color layer.
    value.enableUserInterfaceRecomposition = sl::eFalse;
    value.onErrorCallback = &onApiError;
...
void SlPresenter::Impl::ensure(Image& destination, const D3D12_RESOURCE_DESC& input) {
    auto target = input;
    target.Format = sampleFormat(target.Format);
    target.Flags = D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET;
    target.Layout = D3D12_TEXTURE_LAYOUT_UNKNOWN;
    target.Alignment = 0;
    if (destination.resource) {
        const auto old = destination.resource->GetDesc();
        if (old.Width == target.Width && old.Height == target.Height && old.Format == target.Format)
            return;
    }
    destination.resource.Reset();
    destination.state = D3D12_RESOURCE_STATE_COMMON;
    D3D12_HEAP_PROPERTIES heap{};
    heap.Type = D3D12_HEAP_TYPE_DEFAULT;
    graphicsCheck(runtime->device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &target,
                                                           destination.state, nullptr,
                                                           IID_PPV_ARGS(&destination.resource)),
                  "Create private Streamline input");
}
void SlPresenter::Impl::prepare(Slot& slot, const PresentationFrame& frame, bool generate) {
    if (!slot.allocator) {
        graphicsCheck(runtime->device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT,
                                                              IID_PPV_ARGS(&slot.allocator)),
                      "Create SL input allocator");
        graphicsCheck(runtime->device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT,
        ...
        graphicsCheck(runtime->device->CreateDescriptorHeap(&heap, IID_PPV_ARGS(&slot.srv)),
                      "Create SL HUDless SRV heap");
    ...
    runtime->device->CreateShaderResourceView(slot.rawHudless.resource.Get(), &srv,
                                              slot.srv->GetCPUDescriptorHandleForHeapStart());
    runtime->device->CreateRenderTargetView(slot.hudless.resource.Get(), nullptr,
                                            slot.rtv->GetCPUDescriptorHandleForHeapStart());
}
void SlPresenter::Impl::copy(Slot& slot, Image& destination, const PresentImage& source) {
    ...
    Dx11Dx12::transition(slot.list.Get(), source.resource.Get(), source.state,
                         D3D12_RESOURCE_STATE_COPY_SOURCE);
    slot.list->CopyResource(destination.resource.Get(), source.resource.Get());
    Dx11Dx12::transition(slot.list.Get(), source.resource.Get(), D3D12_RESOURCE_STATE_COPY_SOURCE,
                         source.state);
}
void SlPresenter::Impl::copyFrame(Slot& slot, const PresentationFrame& frame, bool generate) {
    ...
    if (generate) {
        ...
        if (images.slDistortion.resource)
            slot.distortion.transition(slot.list.Get(), D3D12_RESOURCE_STATE_COMMON);
    }
    ...
    graphicsCheck(chain->GetBuffer(index, IID_PPV_ARGS(&backbuffer)),
                  "Get proxy-owned Streamline backbuffer");
    ...
    if (frame.readyValue)
        graphicsCheck(queue->Wait(frame.readyFence.Get(), frame.readyValue),
                      "Wait for immutable production frame");
...
void SlPresenter::Impl::tag(Slot& slot, const PresentationFrame& frame, const ContentView& view) {
    ...
    constants.clipToPrevClip = matrix(frame.clipToPrevClip);
    constants.prevClipToClip = matrix(frame.prevClipToClip);
    constants.jitterOffset = {frame.jitterX, frame.jitterY};
    constants.cameraPinholeOffset = {
        0, 0}; // This adapter supplies its full projection matrix, not an additional pinhole shift.
    const auto motion = slot.motion.resource->GetDesc();
    constants.mvecScale = {frame.motionScaleX / static_cast<float>(motion.Width),
                           frame.motionScaleY / motion.Height};
    constants.cameraPos = vector(frame.cameraPosition);
    constants.cameraUp = vector(frame.cameraUp);
    constants.cameraRight = vector(frame.cameraRight);
    constants.cameraFwd = vector(frame.cameraForward);
    constants.cameraNear = frame.cameraNear;
    constants.cameraFar = frame.cameraFar;
    constants.cameraFOV = frame.verticalFov;
    constants.cameraAspectRatio = view.cameraAspect;
    constants.depthInverted = boolean(frame.depthInverted);
    constants.cameraMotionIncluded = boolean(frame.cameraMotionIncluded);
    constants.motionVectors3D = sl::eFalse;
    constants.reset =
        boolean(frame.reset || forceReset || !haveFrame ||
                frame.applicationFrameId != lastApplicationFrame + 1 || view.area != lastAcceptedArea);
    ...
    detail::slCheck(runtime->api.constants(constants, *slot.ticket->token, sl::ViewportHandle(0u)),
                    "Set exact application-frame constants");
    sl::Resource resources[] = {
        ...
        {sl::ResourceType::eTex2d, slot.distortion.resource.Get(),
         static_cast<uint32_t>(slot.distortion.state)}};
    const auto depth = slot.depth.resource->GetDesc();
    const sl::Extent low{0, 0, static_cast<uint32_t>(depth.Width), depth.Height};
    // H and Final share the complete physical display image, but the SDK sees
    // the same content rectangle in both. D/M, jitter and projection stay in
    // the content-local camera domain; no letterbox scaling is applied twice.
    const auto& area = view.area;
    ...
    sl::ResourceTag tags[] = {
        ...
        {&resources[2], sl::kBufferTypeHUDLessColor, sl::ResourceLifecycle::eValidUntilPresent, &area},
        // Null tags also retire any UI inputs retained by an earlier integration.
        ...
        {frame.images->slDistortion.resource ? &resources[3] : nullptr,
         sl::kBufferTypeBidirectionalDistortionField, sl::ResourceLifecycle::eValidUntilPresent, &distortion},
    ...
    slot.tagged = true;
    detail::slCheck(runtime->api.tags(*slot.ticket->token, sl::ViewportHandle(0u), tags,
                                      static_cast<uint32_t>(std::size(tags)), nullptr),
                    "Tag private native Streamline frame inputs");
}
bool SlPresenter::Impl::eligible(const PresentationFrame& frame, const PresentArguments& arguments,
                                 ...
                                 const std::shared_ptr<detail::SlTicket>& ticket, ContentView& view) {
    auto reject = [this](const char* reason) {
        observation.reason = reason;
        return false;
    };
    if (request.mode == SlGenerationMode::Off) {
    ...
    if (runtime->reflexMode == SlReflexMode::Off)
        return reject("DLSS FG requires Reflex enabled; its before-input sleep is still maintained");
    if (!frame.inputsComplete || frame.generation != creation.generation || !frame.renderWidth ||
        !frame.renderHeight)
    ...
    if (request.mode == SlGenerationMode::Fixed) {
        if (!request.generatedFrames || request.generatedFrames > observation.maximumGeneratedFrames)
            return reject(
                "The requested additional-frame count exceeds this adapter/runtime's reported capability");
    } else if (request.mode == SlGenerationMode::Dynamic) {
        if (!observation.dynamicSupported)
            return reject("Dynamic MFG is unsupported by this adapter/runtime");
        ...
        return reject("Unknown DLSS generation mode");
    ...
    if (!contentArea(frame.generationRect, description.Width, description.Height, view.area))
        return reject("SL generation rectangle is outside the output or has non-positive area");
    if (view.area.width < observation.minimumDimension || view.area.height < observation.minimumDimension)
        return reject("The content rectangle is below this runtime's minimum FG dimension");
    if (description.Format == DXGI_FORMAT_R16G16B16A16_FLOAT ||
        colorSpace == DXGI_COLOR_SPACE_RGB_FULL_G10_NONE_P709)
        return reject(
            "DLSS FG does not support an FP16/scRGB swapchain; color encoding is not silently changed");
    if (frame.colorSpace != colorSpace)
        return reject("Frame and swapchain color encodings differ");
    ...
    const auto& images = *frame.images;
    if (!sizeIs(images.hudless, description.Width, description.Height) ||
        !sizeIs(images.depth, frame.renderWidth, frame.renderHeight) ||
        !sizeIs(images.motion, frame.renderWidth, frame.renderHeight))
        return reject("HUDless or depth/MV domains do not match the immutable frame");
    if (images.slDistortion.resource &&
        (!texture(images.slDistortion) ||
    ...
    if (!frame.cameraMotionIncluded || !std::isfinite(frame.cameraNear) || frame.cameraNear <= 0 ||
        !std::isfinite(frame.cameraFar) || frame.cameraFar <= frame.cameraNear ||
        !std::isfinite(frame.verticalFov) || frame.verticalFov <= 0 || frame.verticalFov >= 3.141593f ||
    ...
    if (!finite(frame.cameraViewToClip) || !finite(frame.clipToCameraView) || !finite(frame.clipToPrevClip) ||
        !finite(frame.prevClipToClip) || !finite(frame.cameraPosition) || !finite(frame.cameraUp) ||
        !finite(frame.cameraRight) || !finite(frame.cameraForward))
        return reject("SL camera matrices/basis contain non-finite values");
    if (!projectionAspect(frame.cameraViewToClip, view.cameraAspect))
        return reject("SL perspective projection has invalid camera aspect scales");
...
HRESULT SlPresenter::Impl::present(FrameLease frame, const PresentArguments& arguments,
    ...
    if (FAILED(asynchronousError.load()))
        throw std::runtime_error("The Streamline asynchronous presentation callback reported failure");
    if (!frame || !frame->applicationFrameId || frame->generation != creation.generation ||
        ...
        sampleFormat(frame->images->finalColor.resource->GetDesc().Format) !=
            sampleFormat(description.Format) ||
        (frame->readyValue && !frame->readyFence))
        return E_INVALIDARG;
    observation.requested = request.mode;
    auto ticket = runtime->claim(frame->applicationFrameId);
    ContentView view;
    bool generate = eligible(*frame, arguments, request, ticket, view);
    if (!generate) {
    ...
    const bool boundaryReady =
        !arguments.boundary || !arguments.boundary->before ||
    ...
    if (generate && (!boundaryReady || !runtime->readyForPresent(slot.ticket))) {
        ...
        forceReset = true;
        observation.reason =
            "Render submission end/Present start were not established at the actual boundary";
    } else if (generate) {
    ...
    ++observation.applicationPresents;
    observation.lastApplicationFrameId = frame->applicationFrameId;
    ...
    if (generate) {
        if (!state.inputsProcessingCompletionFence)
            throw std::runtime_error(
                "DLSS FG did not provide its input-consumption fence; private inputs remain quarantined");
    ...
    for (auto& item : slots)
        if (item.producer && completed(copiedFence.Get(), item.copied))
            item.producer.reset();
    haveFrame = true;
    ...
    if (FAILED(result) && result != DXGI_ERROR_WAS_STILL_DRAWING)
        graphicsCheck(result, "Present Streamline frame");
    return result;
}
PresentRetirement SlPresenter::Impl::stop() noexcept {
    if (stopped)
        return observation.quarantined ? PresentRetirement::Quarantined : PresentRetirement::Drained;
    ...
    try {
        ...
        quarantine(exception.what());
    } catch (...) {
        quarantine("Unknown Streamline presenter retirement failure");
    }
    return PresentRetirement::Quarantined;
}
SlPresenter::SlPresenter(const SlPresenterCreateInfo& info) {
    if (!info.runtime)
        throw SlPresenterCreationError("Streamline runtime is missing", PresentRetirement::Drained);
    ...
    try {
        ...
        if (retirement == PresentRetirement::Quarantined)
            impl_.release();
...
HRESULT SlPresenter::present(FrameLease frame, const PresentArguments& arguments,
                             const SlGenerationRequest& request) noexcept {
    try {
        return impl_->present(std::move(frame), arguments, request);
    } catch (const std::exception& exception) {
        impl_->quarantine(exception.what());
    } catch (...) {
        impl_->quarantine("Unknown Streamline presentation failure");
    }
    return DXGI_ERROR_DEVICE_REMOVED;
}
PresentRetirement SlPresenter::stop() noexcept {
    return impl_->stop();
}
HRESULT SlPresenter::resize(const DXGI_SWAP_CHAIN_DESC1& description, uint64_t generation) noexcept {
    ...
    if (!description.Width || !description.Height || description.SampleDesc.Count != 1 ||
        description.Stereo || description.BufferCount < 2 ||
        description.SwapEffect != impl_->description.SwapEffect ||
        description.Flags != impl_->description.Flags)
        return E_INVALIDARG;
    try {
        impl_->pause();
        // Keep the actual DXGI/Win32 control on the caller's render/UI boundary.
        const auto result =
            impl_->chain->ResizeBuffers(description.BufferCount, description.Width, description.Height,
                                        description.Format, description.Flags);
        if (FAILED(result))
            return result;
        impl_->description = description;
        impl_->creation.generation = generation;
        ...
        impl_->quarantine("Unknown Streamline resize failure");
    }
    return DXGI_ERROR_DEVICE_REMOVED;
}
HRESULT SlPresenter::setFullscreenState(BOOL fullscreen, IDXGIOutput* output) noexcept {
    ...
    try {
        impl_->pause();
        return impl_->chain->SetFullscreenState(fullscreen, output);
    } catch (const std::exception& exception) {
        impl_->quarantine(exception.what());
    } catch (...) {
        impl_->quarantine("Unknown Streamline fullscreen failure");
    }
    return DXGI_ERROR_DEVICE_REMOVED;
}
HRESULT SlPresenter::resizeTarget(const DXGI_MODE_DESC& description) noexcept {
    ...
    try {
        impl_->pause();
        return impl_->nativeChain->ResizeTarget(&description);
    } catch (const std::exception& exception) {
        impl_->quarantine(exception.what());
    } catch (...) {
        impl_->quarantine("Unknown Streamline target resize failure");
    }
    return DXGI_ERROR_DEVICE_REMOVED;
}
HRESULT SlPresenter::setColorSpace(DXGI_COLOR_SPACE_TYPE colorSpace) noexcept {
    ...
    try {
        impl_->pause();
        const auto result = impl_->nativeChain->SetColorSpace1(colorSpace);
        if (SUCCEEDED(result))
            impl_->colorSpace = colorSpace;
        return result;
    } catch (const std::exception& exception) {
        impl_->quarantine(exception.what());
    } catch (...) {
        impl_->quarantine("Unknown Streamline color-space failure");
    }
    return DXGI_ERROR_DEVICE_REMOVED;
}
...
HRESULT SlPresenter::setMaximumFrameLatency(UINT latency) noexcept {
    if (!impl_->usable())
        return DXGI_ERROR_DEVICE_REMOVED;
    if (!(impl_->description.Flags & DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT))
...
IDXGISwapChain4* SlPresenter::swapChainForQueries() const noexcept {
    return impl_->usable() ? impl_->nativeChain.Get() : nullptr;
}
