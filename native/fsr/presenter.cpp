...
struct Module {
    ...
    void load(const std::filesystem::path& path) {
        ...
        if (!handle)
            graphicsCheck(HRESULT_FROM_WIN32(GetLastError()), "Load FSR FG runtime");
    }
};
struct Event {
...
void barrier(ID3D12GraphicsCommandList* list, ID3D12Resource* resource, D3D12_RESOURCE_STATES before,
    ...
    if (before != after)
        Dx11Dx12::transition(list, resource, before, after);
}
...
uint32_t transferFunction(DXGI_COLOR_SPACE_TYPE colorSpace) {
    ...
    case DXGI_COLOR_SPACE_RGB_FULL_G22_NONE_P709:
        return FFX_API_BACKBUFFER_TRANSFER_FUNCTION_SRGB;
    case DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020:
        return FFX_API_BACKBUFFER_TRANSFER_FUNCTION_PQ;
    case DXGI_COLOR_SPACE_RGB_FULL_G10_NONE_P709:
        return FFX_API_BACKBUFFER_TRANSFER_FUNCTION_SCRGB;
    default:
        throw std::invalid_argument("Unsupported FSR presentation color space");
    }
}
...
// Format conversion only: postprocessed HUDless values already use Final's
// presentation encoding. Load does not convert linear values to sRGB/PQ.
// The SDK receives Final plus HUDLessColor and owns UI extraction/composition.
...
ComPtr<ID3DBlob> compileShader(const char* entry, const char* target) {
    ...
    const auto result = D3DCompile(shaders, sizeof(shaders) - 1, "DSPAASR FSR presentation", nullptr, nullptr,
                                   entry, target, D3DCOMPILE_ENABLE_STRICTNESS, 0, &shader, &errors);
    if (FAILED(result))
        throw std::runtime_error(errors ? std::string(static_cast<const char*>(errors->GetBufferPointer()),
                                                      errors->GetBufferSize())
                                        : "Compile presentation shader");
    return shader;
}
} // namespace
...
struct FsrPresenter::Impl {
    ...
    unsigned contextRenderWidth = 0, contextRenderHeight = 0;
    RECT previousGenerationRect{};
    unsigned previousRenderWidth = 0, previousRenderHeight = 0;
    ...
    explicit Impl(const FsrPresenterCreateInfo& info)
        : creation(info), factory(info.factory), device(info.device), queue(info.gameQueue) {}
    void initialize();
    ...
    void configure(ffxContext context, uint64_t id, bool enabled, const PresentationFrame& frame,
                   ID3D12Resource* distortion, ID3D12Resource* hudless = nullptr);
    Slot& acquire();
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE target(ID3D12DescriptorHeap* heap, unsigned index, ID3D12Resource* resource);
    void draw(ID3D12GraphicsCommandList* commands, ID3D12PipelineState* pipeline, ID3D12DescriptorHeap* views,
              unsigned first, D3D12_CPU_DESCRIPTOR_HANDLE destination);
    void copyInput(ID3D12GraphicsCommandList* commands, const PresentImage& source, Image& destination,
                   D3D12_RESOURCE_STATES after);
    ...
    template <class Operation> HRESULT controlChain(Operation operation, bool pause = false) noexcept {
        ...
        try {
            ...
            sdkControl([this] {
                if (fgContext)
                    configure(fgContext, nextFrame, false, PresentationFrame{}, nullptr);
            ...
            quarantine("Unknown FSR swapchain control failure");
        }
        return E_FAIL;
    }
};
...
void FsrPresenter::Impl::initialize() {
    ...
    if (queueDevice.Get() != device.Get())
        throw std::invalid_argument("FSR queue belongs to another device");
    ...
    if (!GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS,
                            reinterpret_cast<LPCWSTR>(&dispatchCallback), &callbackModule.handle))
    ...
    graphicsCheck(device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&fence)),
                  "Create FSR host fence");
    fenceEvent.handle = CreateEventW(nullptr, FALSE, FALSE, nullptr);
    if (!fenceEvent.handle)
        graphicsCheck(HRESULT_FROM_WIN32(GetLastError()), "Create FSR host event");
    auto& selection = chainSelection;
    selection.header.type = FFX_API_DESC_TYPE_OVERRIDE_VERSION;
    selection.versionId = chainId;
    auto& version = chainVersion;
    version.header = {FFX_API_CREATE_CONTEXT_DESC_TYPE_FRAMEGENERATIONSWAPCHAIN_VERSION_DX12,
                      &selection.header};
    ...
    desc.swapchain = chain.GetAddressOf();
    desc.hwnd = creation.window;
    ...
    if (api.Query(&chainContext, &query.header) == FFX_API_RETURN_OK)
        chainMemory = memory.totalUsageInBytes;
}
uint64_t FsrPresenter::Impl::provider(uint64_t type, const char* wanted) {
    ...
    for (size_t i = 0; i < count; ++i)
        if (names[i] && std::strcmp(names[i], wanted) == 0)
            return ids[i];
    throw std::runtime_error(std::string("Required FSR provider ") + wanted +
                             " is unavailable; ML is not substituted");
}
void FsrPresenter::Impl::verifyProvider(ffxContext context, uint64_t expected) {
    ...
    if (query.versionId != expected)
        throw std::runtime_error("FSR ignored the explicitly selected provider");
}
ComPtr<ID3D12PipelineState> FsrPresenter::Impl::pipeline(ID3DBlob* vs, ID3DBlob* ps, DXGI_FORMAT format) {
    ...
    graphicsCheck(device->CreateGraphicsPipelineState(&desc, IID_PPV_ARGS(&result)),
                  "Create FSR HUDless conversion pipeline");
    return result;
}
void FsrPresenter::Impl::pipelines() {
    if (!root) {
        ...
        graphicsCheck(D3D12SerializeRootSignature(&desc, D3D_ROOT_SIGNATURE_VERSION_1, &bytes, &error),
                      "Serialize FSR HUDless conversion root signature");
        graphicsCheck(device->CreateRootSignature(0, bytes->GetBufferPointer(), bytes->GetBufferSize(),
                                                  IID_PPV_ARGS(&root)),
                      "Create FSR HUDless conversion root signature");
    }
...
uint64_t FsrPresenter::Impl::completed() {
    ...
    if (value == UINT64_MAX)
        throw std::runtime_error("FSR presenter fence reports device removal");
    return value;
}
...
void FsrPresenter::Impl::sdkControl(std::function<void()> operation) {
    if (control)
        throw std::runtime_error("A quarantined FSR SDK control operation is still owned");
    ...
    try {
        work->thread = std::thread([work, operation = std::move(operation)]() mutable {
            try {
                ...
                try {
                    work->done.set_exception(std::current_exception());
                } catch (...) {
                }
            }
        });
...
void FsrPresenter::Impl::collect() {
    ...
    for (auto& slot : slots) {
        if (slot.copied && slot.copied <= value)
            slot.lease.reset();
        if (slot.retired && slot.retired <= value)
            slot.occupied = false;
    }
}
...
D3D12_RESOURCE_DESC FsrPresenter::Impl::validate(const PresentImage& image, unsigned width, unsigned height,
                                                 const char* role) const {
    if (!image.resource)
        throw std::invalid_argument(std::string("Missing presentation ") + role);
...
void FsrPresenter::Impl::validateTemporal(const PresentationFrame& frame) const {
    ...
    const auto depth =
        readableFormat(validate(images.depth, frame.renderWidth, frame.renderHeight, "depth").Format);
    ...
    if (!images.motion.resource)
        throw std::invalid_argument("Missing motion vectors");
    const auto motion = images.motion.resource->GetDesc();
    const bool displayMotion = motion.Width == width && motion.Height == height;
    const auto motionFormat =
        readableFormat(validate(images.motion, displayMotion ? width : frame.renderWidth,
                                displayMotion ? height : frame.renderHeight, "motion vectors")
                           .Format);
    ...
    if (images.fsrDistortion.resource) {
        const auto distortion =
            readableFormat(validate(images.fsrDistortion, width, height, "FSR distortion").Format);
    ...
    for (float value : {frame.jitterX, frame.jitterY, frame.motionScaleX, frame.motionScaleY,
        ...
        if (!std::isfinite(value))
            throw std::invalid_argument("Non-finite FSR camera metadata");
    ...
    for (const auto* vector :
         {&frame.cameraPosition, &frame.cameraUp, &frame.cameraRight, &frame.cameraForward}) {
        float length = 0;
        for (const auto value : *vector) {
            if (!std::isfinite(value))
                throw std::invalid_argument("Non-finite FSR camera basis");
            length += value * value;
        }
...
uint32_t FsrPresenter::Impl::flags(const PresentationFrame& frame) const {
    ...
    if (frame.depthInfinite)
        result |= FFX_FRAMEGENERATION_ENABLE_DEPTH_INFINITE;
    if (frame.motionVectorsJittered)
        result |= FFX_FRAMEGENERATION_ENABLE_MOTION_VECTORS_JITTER_CANCELLATION;
    if (frame.colorSpace != DXGI_COLOR_SPACE_RGB_FULL_G22_NONE_P709)
        result |= FFX_FRAMEGENERATION_ENABLE_HIGH_DYNAMIC_RANGE;
    if (creation.debugChecking)
        result |= FFX_FRAMEGENERATION_ENABLE_DEBUG_CHECKING;
...
void FsrPresenter::Impl::configure(ffxContext context, uint64_t id, bool enabled,
    ...
    if (enabled && !hudless)
        throw std::invalid_argument("Enabled FSR generation requires its HUDless slot");
    ...
    configuration.HUDLessColor = enabled ? ffxApiGetResourceDX12(hudless) : FfxApiResource{};
    ...
    configuration.generationRect =
        enabled ? generationRect(frame.generationRect, creation.swapChain.Width, creation.swapChain.Height)
                : FfxApiRect2D{};
    // Final remains in the SDK backbuffer. Its HUDless mode extracts the UI;
    // neither a separately rendered alpha layer nor a UI callback is registered.
...
void FsrPresenter::Impl::ensureContext(const PresentationFrame& frame) {
    ...
    if (fgContext && contextWidth == creation.swapChain.Width && contextHeight == creation.swapChain.Height &&
        contextFlags == wantedFlags && contextRenderWidth >= frame.renderWidth &&
        contextRenderHeight >= frame.renderHeight)
        return;
    try {
        drain();
    } catch (...) {
    ...
    // SDK maxRenderSize independently sizes depth/MV reconstruction resources;
    // displaySize is the optical-flow/output domain, not a raster upper bound.
    next->description.maxRenderSize = {std::max(creation.swapChain.Width, frame.renderWidth),
                                       std::max(creation.swapChain.Height, frame.renderHeight)};
    ...
    checkFfx(api.CreateContext(&next->context, &next->description.header, nullptr),
             "Create analytical FSR 3.1.6 FG");
    ...
    effects.erase(
        std::remove_if(effects.begin(), effects.end(), [](const auto& effect) { return !effect->context; }),
        effects.end());
    contextFlags = wantedFlags;
    contextWidth = creation.swapChain.Width;
    contextHeight = creation.swapChain.Height;
    contextRenderWidth = next->description.maxRenderSize.width;
    contextRenderHeight = next->description.maxRenderSize.height;
    lastEnabled = false;
    ...
    if (api.Query(&fgContext, &query.header) == FFX_API_RETURN_OK)
        fgMemory = memory.totalUsageInBytes;
}
...
ComPtr<ID3D12Resource> FsrPresenter::Impl::texture(unsigned width, unsigned height, DXGI_FORMAT format,
    ...
    graphicsCheck(device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &desc,
                                                  ...
                                                  IID_PPV_ARGS(&result)),
                  "Create private FSR presentation image");
    return result;
}
void FsrPresenter::Impl::allocate(Slot& slot, const PresentationFrame& frame, bool generating) {
    if (!slot.allocator) {
        graphicsCheck(
            device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&slot.allocator)),
            "Create FSR presentation allocator");
        graphicsCheck(device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, slot.allocator.Get(),
                                                nullptr, IID_PPV_ARGS(&slot.commands)),
                      "Create FSR presentation commands");
        graphicsCheck(slot.commands->Close(), "Close initial FSR presentation commands");
        D3D12_DESCRIPTOR_HEAP_DESC heap{};
        heap.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
        heap.NumDescriptors = 1;
        heap.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
        graphicsCheck(device->CreateDescriptorHeap(&heap, IID_PPV_ARGS(&slot.views)),
                      "Create FSR private SRVs");
        heap.Type = D3D12_DESCRIPTOR_HEAP_TYPE_RTV;
        heap.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_NONE;
        graphicsCheck(device->CreateDescriptorHeap(&heap, IID_PPV_ARGS(&slot.targets)),
                      "Create FSR private RTVs");
    }
    const auto width = creation.swapChain.Width, height = creation.swapChain.Height;
    auto ensure = [this](Image& image, unsigned w, unsigned h, DXGI_FORMAT format,
        ...
        if (image.resource) {
            const auto desc = image.resource->GetDesc();
            if (desc.Width == w && desc.Height == h && desc.Format == format && desc.Flags == flags)
                return;
        }
...
void FsrPresenter::Impl::draw(ID3D12GraphicsCommandList* commands, ID3D12PipelineState* pipeline,
    ...
    auto table = views->GetGPUDescriptorHandleForHeapStart();
    table.ptr += static_cast<UINT64>(first) * srvStride;
    commands->SetGraphicsRootDescriptorTable(0, table);
    const D3D12_VIEWPORT viewport{
        0, 0, static_cast<float>(creation.swapChain.Width), static_cast<float>(creation.swapChain.Height),
        0, 1};
    const RECT scissor{0, 0, static_cast<LONG>(creation.swapChain.Width),
                       static_cast<LONG>(creation.swapChain.Height)};
...
void FsrPresenter::Impl::prepare(Slot& slot) {
    ...
    desc.commandList = slot.commands.Get();
    ...
    desc.frameTimeDelta = frame.deltaMilliseconds;
    desc.reset = frame.reset;
...
bool FsrPresenter::Impl::snapshot(uint64_t id, Snapshot& output) {
    ...
    for (const auto& slot : slots) {
        ...
        output.generating = slot.generating;
        return true;
    }
    return false;
}
ffxReturnCode_t FsrPresenter::Impl::dispatchCallback(ffxDispatchDescFrameGeneration* desc,
                                                     void* user) noexcept {
...
HRESULT FsrPresenter::Impl::present(FrameLease frame, const PresentArguments& args, bool generate) {
    ...
    if (args.flags & DXGI_PRESENT_TEST)
        return chain->Present(args.syncInterval, args.flags);
    if (!frame || !frame->images || frame->generation != creation.generation || !frame->applicationFrameId ||
        frame->applicationFrameId <= lastApplicationFrame || (frame->readyValue && !frame->readyFence))
        return E_INVALIDARG;
    const auto final =
        validate(frame->images->finalColor, creation.swapChain.Width, creation.swapChain.Height, "Final");
    if (readableFormat(final.Format) != readableFormat(creation.swapChain.Format))
        return E_INVALIDARG;
    if (dispatchError.load()) {
        fgFaulted = true;
    ...
    allocate(slot, *frame, generate);
    {
        std::lock_guard lock(slotsMutex);
        slot.occupied = true;
        slot.copied = slot.retired = 0;
        nextFrame = frame->applicationFrameId;
        slot.id = nextFrame;
        slot.generating = generate;
        slot.lease = frame;
        slot.metadata = *frame;
        slot.metadata.reset =
            frame->reset || historyReset || !lastEnabled ||
            frame->applicationFrameId != lastApplicationFrame + 1 ||
            (generate &&
             (!sameRect(
                  previousGenerationRect,
    ...
    if (fgContext) {
        auto* distortion =
            generate && frame->images->fsrDistortion.resource ? slot.distortion.resource.Get() : nullptr;
        auto* hudless = generate ? slot.hudless.resource.Get() : nullptr;
        if (generate != lastEnabled) {
            ...
            const ComPtr<ID3D12Resource> retainedDistortion = distortion, retainedHudless = hudless;
            sdkControl([this, id, generate, metadata, retainedDistortion, retainedHudless] {
                configure(fgContext, id, generate, metadata, retainedDistortion.Get(), retainedHudless.Get());
                graphicsCheck(device->GetDeviceRemovedReason(), "FSR mode-change device health");
            });
        } else
            configure(fgContext, slot.id, generate, slot.metadata, distortion, hudless);
    }
    if (frame->readyFence && frame->readyValue) {
        ...
        graphicsCheck(queue->Wait(frame->readyFence.Get(), frame->readyValue),
                      "Wait for FSR presentation producer");
    }
    ComPtr<ID3D12Resource> backbuffer;
    graphicsCheck(chain->GetBuffer(chain->GetCurrentBackBufferIndex(), IID_PPV_ARGS(&backbuffer)),
                  "Get FSR replacement backbuffer");
    ...
    lastEnabled = generate;
    if (generate) {
        previousGenerationRect =
            displayRect(creation.swapChain.Width, creation.swapChain.Height, frame->generationRect);
        previousRenderWidth = frame->renderWidth;
        previousRenderHeight = frame->renderHeight;
    }
    ...
    if (generate && !dispatchError.load())
        reason.clear();
    else if (!requested && !fgFaulted)
        reason.clear();
...
PresentRetirement FsrPresenter::Impl::stop() noexcept {
    ...
    try {
        ...
        quarantine("Unknown FSR retirement failure");
    }
    return PresentRetirement::Quarantined;
}
HRESULT FsrPresenter::Impl::resize(const DXGI_SWAP_CHAIN_DESC1& desc, uint64_t generation) {
    if (stopped || quarantined || !chain)
        return DXGI_ERROR_INVALID_CALL;
    if (desc.SampleDesc.Count != 1 || desc.SampleDesc.Quality || !colorFormat(desc.Format) ||
        desc.BufferCount < 2 || desc.SwapEffect != creation.swapChain.SwapEffect ||
        desc.AlphaMode != creation.swapChain.AlphaMode || desc.Scaling != creation.swapChain.Scaling ||
        desc.BufferUsage != creation.swapChain.BufferUsage)
        return E_INVALIDARG;
    drain();
    // Keep the old effect alive during ResizeBuffers: the SDK may retain its
    // frame-generation callback wrapper until the new effect replaces it safely.
    sdkControl([this, desc] {
        graphicsCheck(
            chain->ResizeBuffers(desc.BufferCount, desc.Width, desc.Height, desc.Format, desc.Flags),
    ...
    contextWidth = contextHeight = 0;
    previous = nullptr;
    lastApplicationFrame = 0;
    fgFaulted = false;
...
FsrPresenter::FsrPresenter(const FsrPresenterCreateInfo& info) : impl_(std::make_unique<Impl>(info)) {
    try {
        ...
        try {
            ...
            throw FsrPresenterCreationError(exception.what(), retirement);
        } catch (...) {
            throw FsrPresenterCreationError("Unknown FSR presenter initialization failure", retirement);
        }
    }
}
...
HRESULT FsrPresenter::present(FrameLease frame, const PresentArguments& arguments, bool generate) noexcept {
    try {
        return impl_->present(std::move(frame), arguments, generate);
        ...
        impl_->quarantine("Unknown FSR presentation failure");
    }
    return E_FAIL;
}
HRESULT FsrPresenter::resize(const DXGI_SWAP_CHAIN_DESC1& description, uint64_t generation) noexcept {
    try {
        return impl_->resize(description, generation);
    } catch (const std::exception& exception) {
        impl_->quarantine(exception.what());
    } catch (...) {
        impl_->quarantine("Unknown FSR resize failure");
    }
    return E_FAIL;
}
PresentRetirement FsrPresenter::stop() noexcept {
    return impl_->stop();
}
HRESULT FsrPresenter::setFullscreenState(BOOL fullscreen, IDXGIOutput* output) noexcept {
    ...
    return impl_->controlChain(
        [fullscreen, retainedOutput](IDXGISwapChain4* chain) {
...
HRESULT FsrPresenter::resizeTarget(const DXGI_MODE_DESC& description) noexcept {
    return impl_->controlChain(
        [description](IDXGISwapChain4* chain) { return chain->ResizeTarget(&description); }, true);
}
HRESULT FsrPresenter::setColorSpace(DXGI_COLOR_SPACE_TYPE colorSpace) noexcept {
    const auto result = impl_->controlChain(
        [colorSpace](IDXGISwapChain4* chain) { return chain->SetColorSpace1(colorSpace); }, true);
    if (SUCCEEDED(result)) {
        impl_->colorSpace = colorSpace;
        impl_->historyReset = true;
    }
    return result;
}
HRESULT FsrPresenter::setHdrMetadata(DXGI_HDR_METADATA_TYPE type, UINT size, void* data) noexcept {
    return impl_->controlChain(
        [type, size, data](IDXGISwapChain4* chain) { return chain->SetHDRMetaData(type, size, data); });
}
HRESULT FsrPresenter::setMaximumFrameLatency(UINT latency) noexcept {
    return impl_->controlChain(
        [latency](IDXGISwapChain4* chain) { return chain->SetMaximumFrameLatency(latency); });
}
HRESULT FsrPresenter::setSourceSize(UINT width, UINT height) noexcept {
    return impl_->controlChain(
        [width, height](IDXGISwapChain4* chain) { return chain->SetSourceSize(width, height); });
}
...
HRESULT FsrPresenter::setMatrixTransform(const DXGI_MATRIX_3X2_F& matrix) noexcept {
    return impl_->controlChain(
        [matrix](IDXGISwapChain4* chain) { return chain->SetMatrixTransform(&matrix); });
}
...
FsrPresenterStatus FsrPresenter::status() const {
    ...
    result.algorithm = "3.1.6 analytical";
    result.swapChainVersion = "3.1.7";
    result.reason = impl_->reason;
    if (impl_->dispatchError.load() && result.reason.empty())
    ...
    result.frameGenerationMemoryBytes = impl_->fgMemory;
    result.swapChainMemoryBytes = impl_->chainMemory;
    result.generationActive =
        impl_->lastEnabled && !impl_->dispatchError.load() && !impl_->quarantined && !impl_->stopped;
    result.providerVerified = impl_->contextVerified;
    result.quarantined = impl_->quarantined;
    return result;
}
} // namespace dspaa
