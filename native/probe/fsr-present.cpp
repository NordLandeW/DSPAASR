...
LONG WINAPI crashTrace(EXCEPTION_POINTERS* failure) {
    ...
    if (SymInitialize(process, "build/native/Release", TRUE)) {
        ...
        for (unsigned i = 0; i < 40; ++i) {
            ...
            else std::fprintf(stderr, "  %s+%llx (%llx)\n", module.ModuleName,
...
struct HiddenWindow {
    ...
    HiddenWindow() {
        WNDCLASSW type{};
        type.lpfnWndProc = DefWindowProcW; type.hInstance = module; type.lpszClassName = className;
        ...
        handle = CreateWindowExW(0, className, className, WS_POPUP, 0, 0, 640, 360,
...
struct Inputs {
    ...
    Inputs(dspaa::Dx11Dx12& bridge, unsigned width, unsigned height) {
        auto access = bridge.lock();
        auto create = [&](dspaa::PresentImage& destination, DXGI_FORMAT format, const void* data, UINT pitch) {
            auto texture = bridge.texture(width, height, format, false);
        ...
        std::vector<uint32_t> final(static_cast<size_t>(width) * height, 0xff1933ccu);
        std::vector<HALF> hudless(static_cast<size_t>(width) * height * 4);
        std::vector<HALF> coverage(static_cast<size_t>(width) * height, XMConvertFloatToHalf(.5f));
        std::vector<float> depth(static_cast<size_t>(width) * height, .5f);
        std::vector<HALF> motion(static_cast<size_t>(width) * height * 2, 0);
        for (size_t i = 0; i < hudless.size(); i += 4) {
            hudless[i] = XMConvertFloatToHalf(.2f);
            hudless[i + 1] = XMConvertFloatToHalf(.6f);
            hudless[i + 2] = XMConvertFloatToHalf(.8f);
            hudless[i + 3] = XMConvertFloatToHalf(1.f);
        }
        // F != H and G/B residuals are negative. These execute the production
        // signed-residual path, but callback counts are NOT a pixel-fidelity test.
        create(images->finalColor, DXGI_FORMAT_R8G8B8A8_UNORM, final.data(), width * sizeof(uint32_t));
        create(images->hudless, DXGI_FORMAT_R16G16B16A16_FLOAT, hudless.data(), width * 4 * sizeof(HALF));
        create(images->occlusionAlpha, DXGI_FORMAT_R16_FLOAT, coverage.data(), width * sizeof(HALF));
        create(images->depth, DXGI_FORMAT_R32_FLOAT, depth.data(), width * sizeof(float));
        create(images->motion, DXGI_FORMAT_R16G16_FLOAT, motion.data(), width * 2 * sizeof(HALF));
        create(images->fsrDistortion, DXGI_FORMAT_R16G16_FLOAT, motion.data(), width * 2 * sizeof(HALF));
        bridge.handoffTo12();
    }
};
} // namespace
int wmain(int argc, wchar_t** argv) {
    ...
    if (argc != 2) {
        std::cerr << "Usage: fsr-present-probe <official-runtime-directory>\n"
                     "Opt-in hidden-window real-adapter lifecycle/ABI probe. Not a display-FPS or image-quality test.\n";
        return 2;
    }
    ...
    try {
        ...
        } else std::cout << "d3d12_debug=unavailable\n";
        ComPtr<IDXGIFactory1> factory;
        dspaa::graphicsCheck(CreateDXGIFactory1(IID_PPV_ARGS(&factory)), "Create FSR presentation probe factory");
        ...
        dspaa::graphicsCheck(D3D11CreateDevice(adapter.Get(), D3D_DRIVER_TYPE_UNKNOWN, nullptr, 0, levels, 2,
                           D3D11_SDK_VERSION, &device, nullptr, nullptr), "Create synthetic D3D11 game device");
        ...
        create.window = window.handle; create.factory = factory.Get();
        create.device = bridge.device12(); create.gameQueue = bridge.queue12();
        create.runtimeDirectory = std::filesystem::absolute(argv[1]); create.generation = 1;
        auto& desc = create.swapChain;
        desc.Width = 640; desc.Height = 360; desc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
        ...
        if (SUCCEEDED(factory.As(&featureFactory)) &&
            SUCCEEDED(featureFactory->CheckFeatureSupport(DXGI_FEATURE_PRESENT_ALLOW_TEARING, &tearing, sizeof(tearing))) && tearing)
        ...
        for (unsigned stage = 0; stage < sizes.size(); ++stage) {
            ...
            if (stage) dspaa::graphicsCheck(presenter.resize(desc, stage + 1), "Resize FSR probe presenter");
            Inputs inputs(bridge, desc.Width, desc.Height);
            const uint64_t readyValue = stage + 1;
            dspaa::graphicsCheck(bridge.queue12()->Signal(ready.Get(), readyValue), "Publish probe producer images");
            const auto before = presenter.status();
            for (unsigned index = 0; index < 24; ++index) {
                messages();
                auto frame = std::make_shared<dspaa::PresentationFrame>();
                frame->images = inputs.images; frame->readyFence = ready; frame->readyValue = readyValue;
                if (index == 13) frameId += 3;
                frame->generation = stage + 1; frame->applicationFrameId = ++frameId;
                frame->inputsComplete = true; frame->reset = index == 0;
                frame->renderWidth = desc.Width; frame->renderHeight = desc.Height;
                frame->cameraNear = .1f; frame->cameraFar = 1000;
                frame->verticalFov = 1.04719755f; frame->deltaMilliseconds = 1000.f / 60;
                ...
                if (index == 12) frame->inputsComplete = false;
                if (!stage && !index) firstLease = frame;
                dspaa::graphicsCheck(presenter.present(frame, {}, enabled), "Present FSR probe frame");
                if (!index) {
                    require(presenter.present(frame, {}, enabled) == E_INVALIDARG, "A repeated simulation token was consumed twice");
                    auto invalid = std::make_shared<dspaa::PresentationFrame>(*frame);
                    invalid->applicationFrameId = 0;
                    require(presenter.present(invalid, {}, enabled) == E_INVALIDARG, "A missing simulation token was accepted");
                }
                const auto observed = presenter.status();
                require(!observed.quarantined, "FSR probe owner was quarantined");
                if (!enabled || !frame->inputsComplete)
                    require(!observed.generationActive, "Incomplete/Off frame still enabled FSR generation");
            ...
            require(after.providerVerified && after.successfulDispatches > before.successfulDispatches &&
                    after.realCallbacks > before.realCallbacks && after.generatedCallbacks > before.generatedCallbacks,
                    "Signed FSR dispatch/presentation callbacks were not observed");
            require(after.applicationPresents - before.applicationPresents == 24, "FSR application frame accounting changed");
            ...
            std::cout << "stage=" << stage << " size=" << desc.Width << 'x' << desc.Height
                      ...
                      << " dispatches=" << after.successfulDispatches << " real_callbacks=" << after.realCallbacks
                      << " generated_callbacks=" << after.generatedCallbacks << " reason=" << after.reason << '\n';
        }
        ...
        require(presenter.stop() == dspaa::PresentRetirement::Drained, "Repeated FSR stop changed retirement");
        ...
        std::cout << "stages=3 gpu_retirement=complete producer_lease=released d3d12_errors=" << errors
                  << " visible_windows=0 display_fps=not_measured pixel_fidelity=not_measured\n";
        return 0;
    ...
    try { (void)debugErrors(diagnostics.Get()); } catch (...) {}
    return 1;
}
